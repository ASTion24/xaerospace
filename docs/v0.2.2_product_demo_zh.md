...
| 配置任意 OpenAI-compatible API | [第 3 章：Provider 系统](#3-v022-provider-系统) |
| 手动运行火箭、轨道、飞机或姿态任务 | [第 10～11 章](#10-demo-6不用-llm-手动运行火箭) |
| 查看方程、模型与真实结果 | [第 12 章：查看动力学方程](#12-demo-8查看动力学方程) |
| 恢复或删除历史工作流 | [第 4 章：Web 工作台](#45-web-工作台完整操作面) |
...
flowchart LR
    ...
    JS --> RESULT
    BS --> RESULT
    RESULT --> STORE[WorkflowStore + SQLite 索引]
    STORE --> OUTPUT[状态 / 事件 / 指标 / 方程 / 图表 / 审计产物]
```
...
|---|---|---|
| `rocket_flight` | `xaerospace.family.rocket_flight.v3` | `xaerospace.scenario.v1` |
| `launch_to_orbit` | `xaerospace.family.launch_to_orbit.v1` | `xaerospace.launch_to_orbit.v1` |
| `orbit_propagation` | `xaerospace.family.orbit_propagation.v2` | `xaerospace.orbit_propagation.v2` |
| `aircraft_flight` | `xaerospace.family.aircraft_flight.v3` | `xaerospace.aircraft_flight.v1` |
| `spacecraft_gnc` | `xaerospace.family.spacecraft_gnc.v3` | `xaerospace.spacecraft_attitude.v1` |
...
- `xaerospace.workflow.v1` 导出；
- 显式 `confirmed=true` 重放；
- 新 workflow ID 和独立结果产物；
- SQLite 持久任务索引；
- 服务重启后的历史、队列和结果恢复；
- 未完成任务显式转为 `interrupted`；
- 产物 SHA-256 完整性校验；
- 终态工作流确认删除。

系统不会自动恢复或重试中断的物理任务。`interrupted` 表示前一服务进程没有
完成终态提交，用户应先检查已保存上下文，再决定是否导出或显式重放。Assistant
...
## 3. v0.2.2 Provider 系统

> [!NOTE]
> 运行时配置、安装资源、Schema 和浏览器存储均使用 `Xaerospace`
> 命名空间。命令行入口仅为 `xaerospace`。
...
flowchart TD
    ...
    ENV[XAEROSPACE_PROVIDER_PROFILE] --> PICK
...
```json
{
  ...
  "providers": {
    "cloud": {
      ...
      "api_key_env": "XAEROSPACE_PROVIDER_API_KEY",
      "header_env": {
        "X-Organization": "XAEROSPACE_PROVIDER_ORGANIZATION"
      },
...
其中 `runtime/tudat/` 保存锁定的 TudatPy 环境，`workspace.sqlite3` 保存工作流
索引和小型状态快照，`runs/` 保存大型时序结果与图表产物。SQLite 使用 WAL、
`synchronous=FULL` 和显式 Schema 版本；遇到高于当前程序的数据库版本时会
Fail-closed，不猜测迁移。

安装到 wheel 后不会向 `site-packages` 或其相邻目录写入数据。需要迁移目录时，
...
```bash
export XAEROSPACE_PROVIDER_CONFIG="$PWD/config/providers.my-team.local.json"
export XAEROSPACE_PROVIDER_PROFILE="team_gateway"
xaerospace web
```
...
```text
CLI --provider-config
> XAEROSPACE_PROVIDER_CONFIG
> 自动发现本地配置

CLI --provider-profile
> XAEROSPACE_PROVIDER_PROFILE
> active_provider
```
...
| `GET` | `/api/workflows?limit=50&offset=0&status=completed` | 分页查询历史 |
| `GET` | `/api/workflows/{id}` | 查询状态 |
| `GET` | `/api/workflows/{id}/export` | 导出工作流 |
| `DELETE` | `/api/workflows/{id}` | 删除终态工作流及产物 |
...
- WorkflowStore 已跨服务重启持久化；DraftSession 仍是最长 1 小时的进程内
  编辑上下文，服务重启后不会恢复对话全文；
- 发射任务使用预设俯仰程序，不是闭环制导；
- 本版本不用于认证或任务安全结论。
...
- 当前前端队列必须为空；
- `workflow_schema` 必须是 `xaerospace.workflow.v1`；
- `tasks` 不能为空；
- 原始合同没有被手工改坏；
...
### 22.11 工作流显示 `interrupted`

这表示服务进程在任务写入终态前停止。系统保留已持久化的合同、状态和完整产物，
但不会自动续跑或重试。先从“历史”打开记录并检查任务错误；需要再次执行时，
导出后显式重放，系统会创建新的 workflow ID 和独立产物目录。

若产物显示 `missing` 或 `corrupt`，下载接口会拒绝返回文件。不要手工覆盖哈希或
修改 `workspace.sqlite3`；应保留原记录用于审计，并重新执行任务。
